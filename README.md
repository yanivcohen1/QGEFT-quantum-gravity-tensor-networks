...
2. **Metric Invariance Tests:** The exact geometric structure is highly sensitive to the heuristic mapping $d(i,j) = -\log(E_{ij})$. A critical milestone is testing whether the same dimensional diagnostics survive under inequivalent but reasonable alternatives such as $E_{ij}^\alpha$, $\log(1+E_{ij})$, rank-based distances, and mutual-information-based metrics. Without this, the metric prescription itself may be doing most of the geometric work.
3. **Breaking the Built-In 3D Prior:** The most important structural test is to start from graph families that are not already approximately 3D, such as trees, hyperbolic constructions, or non-Euclidean sparse priors, and ask whether the same 3D-like diagnostics still appear. If they do not, the present results are better understood as diffusion on a graph family with 3D locality written in from the start.
